...
**id.json path resolution — decided by `Paths.mjs` since 2026-09-21 (`75c48bc`).** `ID_JSON_PATH` is `~/id.json` — one directory *above* `~/flairnode` — in BOTH on-disk layouts: the flat layout (`~/flairnode/FlairNode.js`, every unit built before the golden image) and the release layout (`~/flairnode/releases/<version>/`, `~/flairnode/current` symlink, `~/flairnode/shared/`). `Paths.mjs` tells the layouts apart from the directory structure itself (is the code in a folder whose parent is named `releases`?), never from `process.cwd()` and never from a setting, so nothing on a new card has to be configured for identity to be found. Outside anything an update replaces, so an update can never wipe device identity. `IdManager.mjs` re-exports the constant. History: the original `'../'` path was cwd-relative and broke unless FlairNode was started from `~/flairnode`; 6f6bd3c (2026-07-22) anchored it to the module's own location, which the release layout would then have broken again — `Paths.mjs` is the fix for both. Written `0600` since 1.1.4 (it holds the claim PIN).
...
- `FLAIR_BASE_HOST` env override is baked into this unit's saved pm2 process, pointed at the staging cloud. **Firmware default (no override) is `https://device.flairled.com`** since 1.1.3 (`EnvironmentConfig.mjs`; John's ruling 2026-09-22 — the one address every golden-image card talks to, and the tunnel enrollment's default too). Before 1.1.3 the default was `https://flairled.com`, the legacy droplet. Never assume what a unit talks to without checking its pm2 env first: an override, if set, wins.
...
- **Kiosk units should never show a cursor — root cause confirmed on hardware, fixed via labwc's HideCursor.**

  **Root cause (confirmed on FN-0000001 and FN-00009, Raspberry Pi OS Bookworm, labwc 0.9.8):** the Pi 5's HDMI CEC receivers register as POINTER devices. In `/proc/bus/input/devices`, `vc4-hdmi-0` and `vc4-hdmi-1` report `PROP=20` (`INPUT_PROP_POINTING_STICK`), `EV_REL`, and `REL=3` (`REL_X` + `REL_Y`). libinput sees a pointer, wlroots creates a cursor, labwc draws the arrow, and nothing ever moves it because no CEC remote sends motion. This happens **with no mouse attached** and is intrinsic to the Pi 5 HDMI driver — not a stray peripheral, not a session misconfiguration.

  **The compositor is labwc**, launched by Raspberry Pi OS through `/usr/bin/labwc-pi` with `-m` (`--merge-config`). That merge behavior is load-bearing: the minimal `~/.config/labwc/rc.xml` the kiosk script writes *merges* with `/etc/xdg/labwc/rc.xml` rather than replacing it, so adding one keybind doesn't discard the distro's config.

  **The fix:** `flairnode-kiosk.sh` installs a labwc keybind bound to `HideCursor` + `WarpCursor` (idempotent — written only if `rc.xml` is absent, and it warns rather than clobbers if a user `rc.xml` exists without the keybind), then presses that keybind once at startup with `wtype`. One shot is permanent in practice: the cursor only returns on real pointer input, and nothing on a production wall generates any — which is why no `swayidle`-style idle daemon is needed. `wtype` is a required apt dependency on Pi 5 units (see README section 4); without it the script warns and the cursor stays visible.

  **`render.html`'s `cursor: none` CSS is correct and stays — but it only covers X11.** Under Wayland the compositor draws the cursor; Chromium can only override it via `wl_pointer.set_cursor` in response to a pointer event on its surface, and a pointer that never moves never triggers that path. Do not delete the CSS rule and do not expect it to do anything on a Pi 5.

  **Rejected, tested on hardware — do not re-attempt:** a udev rule setting `LIBINPUT_IGNORE_DEVICE=1` on the `vc4-hdmi` devices. It was applied and the cursor remained.

  **The `$XDG_SESSION_TYPE = x11` / `unclutter` branch never fires on a Pi 5** and must not be counted as Pi 5 cursor coverage. It is retained solely for legacy X11 units. `unclutter` is deliberately not installed on Pi 5 units. Do not switch a unit's session type to X11 to make it apply — that trades a cosmetic defect for a platform regression.
