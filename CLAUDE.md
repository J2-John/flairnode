...
- **Serial number padding inconsistent (`FN-0000001` vs `FN-00005`) — cloud-side, not firmware.** `flairnode` has no serial-formatting code anywhere (verified by grep — `writeIdFile()`/`pushDisplay()` pass `serial_number` through verbatim from the cloud's `/provision` response). The inconsistency is entirely `flairled`'s serial generation. Already ticketed against `flairled`; 7-digit normalization must deploy there before new units mint serials, to avoid perpetuating the mixed-width pool.
